...
def main():
    """Main training function for simple model."""
    
    # Simple configuration
    config = {
        'model': {
            'input_dim': 80,  # Mel features
            'hidden_channels': [128, 128, 256, 256, 512],  # Smaller than original
            'kernel_size': 3,
            'num_speakers': 4,  # Fixed for simplicity
            'dropout': 0.2
        },
        'optimizer': {
            'lr': 1e-3,
    ...
    try:
        print("🔄 Creating VoxConverse dataloaders...")
        ...
        # Initialize trainer
        trainer = SimpleDiarizationTrainer(config)
        ...
        print("💡 Make sure VoxConverse dataset is accessible")
        import traceback
        traceback.print_exc()

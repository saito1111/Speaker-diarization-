...
def prepare_ami(
    ...
    logger.info("Starting prepare_ami")
    logger.debug(f"data_folder: {data_folder}")
    logger.debug(f"manual_annot_folder: {manual_annot_folder}")
    logger.debug(f"save_folder: {save_folder}")
    logger.debug(f"ref_rttm_dir: {ref_rttm_dir}")
    logger.debug(f"meta_data_dir: {meta_data_dir}")
    logger.debug(f"split_type: {split_type}")
    logger.debug(f"skip_TNO: {skip_TNO}")
    logger.debug(f"mic_type: {mic_type}")
    logger.debug(f"vad_type: {vad_type}")
    logger.debug(f"max_subseg_dur: {max_subseg_dur}")
    ...
    logger.debug(f"meta_files: {meta_files}")
    ...
    if not os.path.exists(save_folder):
        ...
        logger.debug(f"Created save_folder: {save_folder}")

    # Setting output option files.
    opt_file = "opt_ami_prepare." + mic_type + ".pkl"
    logger.debug(f"opt_file: {opt_file}")
    ...
    logger.info("Creating meta-data file for the AMI Dataset..")

    # Get the split
    train_set, dev_set, eval_set = get_AMI_split(split_type)
    logger.debug(f"train_set: {train_set}")
    logger.debug(f"dev_set: {dev_set}")
    logger.debug(f"eval_set: {eval_set}")
    ...
    if not os.path.exists(ref_rttm_dir):
        ...
        logger.debug(f"Created ref_rttm_dir: {ref_rttm_dir}")

    # Create reference RTTM files
    splits = ["train", "dev", "eval"]
    for i in splits:
        rttm_file = ref_rttm_dir + "/fullref_ami_" + i + ".rttm"
        logger.debug(f"Creating RTTM file for split: {i}, file: {rttm_file}")
        if i == "train":
    ...
    if not os.path.exists(meta_data_dir):
        ...
        logger.debug(f"Created meta_data_dir: {meta_data_dir}")

    for i in splits:
        rttm_file = ref_rttm_dir + "/fullref_ami_" + i + ".rttm"
        meta_filename_prefix = "ami_" + i
        logger.debug(f"Preparing metadata for split: {i}, RTTM file: {rttm_file}")
        prepare_metadata(
            rttm_file,
            meta_data_dir,
            data_folder,
    ...
    logger.info(f"Saved configuration to {save_opt_file}")
    logger.info("Finished prepare_ami")
...
def prepare_segs_for_RTTM(
    ...
    for main_meet_id in list_ids:
        ...
        logger.debug(f"Looking for sessions in: {audio_dir}/{main_meet_id}*")
        list_sessions.sort()
        logger.debug(f"Processing sessions: {list_sessions}")

        for sess in list_sessions:
            rec_id = os.path.basename(sess)
            path = os.path.join(annot_dir, "segments", rec_id)
            f = path + ".*.segments.xml"
            logger.debug(f"Looking for XML files with pattern: {f}")
            list_spkr_xmls = glob.glob(f)
            list_spkr_xmls.sort()
            logger.debug(f"Found speaker XML files: {list_spkr_xmls}")

            if not list_spkr_xmls:
                logger.warning(f"No XML files found for session: {sess}")
    ...
    logger.debug(f"Writing RTTM to file: {out_rttm_file}")
